...
<?php
    ...
    $sql = "select * from hanghoa where status = 1";
...
<?php
	...
	if(isset($_POST['timkiem'])){
		...
		if(empty($tkma) && empty($tkt) && empty($tkcheck)){
			...
			$sql = "SELECT * FROM hanghoa WHERE status = '$tkcheck'";
			$dataListHH = $db->getData($sql);
		}else if(empty($tkt) && empty($tkcheck)){
			$sql = "SELECT * FROM hanghoa WHERE maHang LIKE '%$tkma%'";
			$dataListHH = $db->getData($sql);
		}else if(empty($tkma) && empty($tkcheck)){
			$sql = "SELECT * FROM hanghoa WHERE tenHang LIKE '%$tkt%'";
			$dataListHH = $db->getData($sql);
		}else if(empty($tkma)){
			$sql = "SELECT * FROM hanghoa WHERE tenHang LIKE '%$tkt%' AND status = $tkcheck";
			$dataListHH = $db->getData($sql);
		}else if(empty($tkt)){
			$sql = "SELECT * FROM hanghoa WHERE maHang LIKE '%$tkma%' AND status = $tkcheck";
			$dataListHH = $db->getData($sql);
		}else if(empty($tkcheck)){
			$sql = "SELECT * FROM hanghoa WHERE maHang LIKE '%$tkma%' AND tenHang LIKE '%$tkt%'";
			$dataListHH = $db->getData($sql);
		}
	}
?>
	...
	<div class="wrapper">
		...
		<div class="nav d-flex">
			...
			<!-- <div class="nav__search">
				...
				<input class="search" type="text" name="nhap"
					placeholder="What you looking for ...">
					 <input type="submit" name="search" class="fa fa-search" aria-hidden="true"></input>
				</form>
			</div> -->
		...
		<div class="table">
			<div class="table__txt d-flex">
				...
				<ul class="table__txt--link">
					<li class="table__txt--item"><a href="goods.php">Danh sách có <?php echo $numHH;?> hàng hóa</a></li>
				</ul>
			</div>
			<div class="table__top d-flex">
				...
				<div class="table__top--add">
					<i class="fa fa-plus-circle" aria-hidden="true"></i><a class="btn"
						href="./goods/add-goods.php">Tạo mới</a>
				</div>
			</div>
			<span> <label
					...
					<tbody>
						<?php foreach($dataListHH as $row){?>
							<tr>
								...
								<?php } ?>
								<td class="edit"><a href="./goods/edit_goods.php?id=<?php echo $row['id'];?>">
										<button id="edit">Sửa</button>
								</a></td>
								<td class="delete"><a 
								href="./goods/delete-goods.php?id=<?php echo $row['id'];?>"
								onclick="return window.confirm('Bạn có muốn xóa <?php echo $row['tenHang'];?> không?');">
