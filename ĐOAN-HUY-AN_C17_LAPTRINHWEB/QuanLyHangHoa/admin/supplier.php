...
<?php
    ...
    $sql = "select * from nhacungcap where status = 1";
    $dataListHH = $db->getData($sql);
    $numHH = count($dataListHH);
?>
<?php
	 $sqlQQ = "select * from nhacungcap where status = 2";
...
<?php
	...
	if(isset($_POST['timkiem'])){
		...
		if(empty($tkma) && empty($tkt) && empty($tkcheck)){
			...
			$sql = "SELECT * FROM nhacungcap WHERE status = '$tkcheck'";
			$dataListHH = $db->getData($sql);
		}else if(empty($tkt) && empty($tkcheck)){
			$sql = "SELECT * FROM nhacungcap WHERE maNCC LIKE '%$tkma%'";
			$dataListHH = $db->getData($sql);
		}else if(empty($tkma) && empty($tkcheck)){
			$sql = "SELECT * FROM nhacungcap WHERE tenNCC LIKE '%$tkt%'";
			$dataListHH = $db->getData($sql);
		}else if(empty($tkma)){
			$sql = "SELECT * FROM nhacungcap WHERE tenNCC LIKE '%$tkt%' AND status = $tkcheck";
			$dataListHH = $db->getData($sql);
		}else if(empty($tkt)){
			$sql = "SELECT * FROM nhacungcap WHERE maNCC LIKE '%$tkma%' AND status = $tkcheck";
			$dataListHH = $db->getData($sql);
		}else if(empty($tkcheck)){
			$sql = "SELECT * FROM nhacungcap WHERE maNCC LIKE '%$tkma%' AND tenNCC LIKE '%$tkt%'";
			$dataListHH = $db->getData($sql);
		}
	}
?>
	...
	<div class="wrapper">
		...
		<div class="sidebar">
			<ul class="sidebar__nav">
				...
				<c:url value="/admin/user" var="UserURL"></c:url>
				<li class="sidebar__nav--item"><a class="sidebar__nav--link"
					href="user.php">
						<div class="sidebar__nav--icon">
				...
				<li class="sidebar__nav--item" data-block="id1"><a
					...
					<div class="sub__men1" id="id1">
						<ul class="list__sub">
							...
							<c:url value="/admin/statistical/import" var="ImportURL"></c:url>
							<li class="sub__menu--item"><a class="sidebar__nav--link"
								href="import.php">
									<div class="sidebar__nav--icon">
							...
							<c:url value="/admin/statistical/export" var="ExportURL"></c:url>
							<li class="sub__menu--item"><a class="sidebar__nav--link"
								href="export.php">
									<div class="sidebar__nav--icon">
		...
		<div class="table">
			<div class="table__txt d-flex">
				<div class="table__txt--title">
					<h2>Danh sách Nhà C.Cấp</h2>
				</div>
				<ul class="table__txt--link">
					<li class="table__txt--item"><a style="font-size: 16px; color: black;" href="supplier.php">Đang giao dịch <?php echo $numHH;?>/</a></li></br>
					<li class="table__txt--item"><a style="font-size: 16px; color: black;" href="supplier.php">Ngừng giao dịch <?php echo $numHHQQ;?></a></li>
				</ul>
			</div>
			...
			<form action="" method="POST">
					<div class="table__top--filter">
						<input style="width: 300px; height: 20px;" type="text"
							name="matk" value="" placeholder="Nhập mã hàng">
						<div class="table__search" style="margin: 1rem 0;">
							<input style="width: 300px; height: 20px;" type="text"
								name="tentk" value="" placeholder="Nhập tên hàng">
				...
				<div class="table__top--add">
					<i class="fa fa-plus-circle" aria-hidden="true"></i><a
						class="btn btn-link1" href="./supplier/add-supplier.php">Tạo mới</a>
				</div>
			</div>
				...
				<table>
					...
					<?php foreach($dataListHH as $row){?>
							<tr>
								...
								<?php } ?>
								<td class="edit"><a href="./supplier/edit-supplier.php?id=<?php echo $row['id'];?>">
										<button id="edit">Sửa</button>
								</a></td>
								<td class="delete"><a 
								href="./supplier/delete-supplier.php?id=<?php echo $row['id'];?>"
								onclick="return window.confirm('Bạn có muốn xóa <?php echo $row['tenNCC'];?> không?');">
