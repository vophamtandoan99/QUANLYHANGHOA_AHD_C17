...
?>
<?php 
      $getDL = "SELECT * FROM hanghoa WHERE id = '$_GET[id]'";
...
<?php
      ...
      if(isset($_POST['btn_edit_goods'])){
        ...
        if(empty($mahang) || empty($tenhang)){
          ...
          $sql1 = "SELECT * FROM hanghoa WHERE maHang = '$mahang'" ;
          $kq = $db->query($sql1);
          $rowkl = mysqli_num_rows($kq);
          if($maHH == $mahang ){
            $sqlEHH = "UPDATE hanghoa SET maHang='$mahang', tenHang='$tenhang', status='$status' WHERE id = '$_GET[id]'";
              $db->update($sqlEHH);
              echo '<script>alert("Sửa thành công!"); window.location="../goods.php";</script>';
            }else if($rowkl > 0){
                 $loii = "Mã hàng đã tồn tại";
            }else{
              $sqlEHH = "UPDATE hanghoa SET maHang='$mahang', tenHang='$tenhang', status='$status' WHERE id = '$_GET[id]'";
              $db->update($sqlEHH);
              echo '<script>alert("Sửa thành công!"); window.location="../goods.php";</script>';
  ...
  <body class="body">
    ...
    <div class="nav d-flex">
      ...
      <div class="table">
        ...
        <h2>SỬA HÀNG HÓA</h2>
        <div class="form--add">
          <div class="form"><i class="fa fa-pencil" aria-hidden="true"></i>
            <div class="form__file">
              <lable for="mahang">Mã Hàng </lable>
              <input name="mahang" type="text" placeholder="Mã hàng" value="<?php echo $maHH;?>" required>
            </div>
            <div class="form__file">
              <lable for="namehang">Tên Hàng </lable>
              <input name="tenhang" type="text" placeholder="Tên hàng" value="<?php echo $tenHH;?>" required>
            </div>
            <div class="form__file">
								<lable for="phone">Trạng thái</lable>
								<select name="status">
										<?php if($th == 1){ ?>
            ...
            <div class="add-link">
              <button name="btn_edit_goods" id="add">Sửa</button>
              <button id="cancel"><a href="../goods.php">Thoát</a></button>
