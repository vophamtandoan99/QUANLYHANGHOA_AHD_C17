      ...
      if(isset($_POST['btn'])){
        ...
        if(empty($mahang) || empty($tenhang)){
          ...
          $rowkl = mysqli_num_rows($kq);
            if($rowkl > 0){
              $loii = "Mã hàng hóa đã tồn tại!";
            }else{
              $sql2 = "insert into hanghoa (maHang, tenHang, status) values ('$mahang', '$tenhang', '1')";
              $db->query($sql2);
              echo '<script>alert("Thêm hàng hóa thành công"); window.location="../goods.php"; </script>';
            }
        }
      }
?>
...
<script src="https://kit.fontawesome.com/58c378b9ed.js"
  ...
  <body class="body">
    ...
    <div class="nav d-flex">
      ...
      <div class="table">
        <form action="add-goods.php" method="POST">
        <h2>THÊM HÀNG HÓA</h2>
        <div class="form--add">
          <div class="form"><i class="fa fa-pencil" aria-hidden="true"></i>
            <div class="form__file">
              <lable for="mahang">Mã Hàng </lable>
              <input name="mahang" type="text" placeholder="Mã hàng" value="" required>
            </div>
            <div class="form__file">
              <lable for="namehang">Tên Hàng </lable>
              <input name="tenhang" type="text" placeholder="Tên hàng" value="" required>
            </div>
            <div class="add-link">
              <button name="btn" id="add">Thêm</button>
              <button id="cancel"><a style="color: black;" href="../goods.php">Thoát</a></button>
