...
?>
<?php 
    $getDL = "SELECT * FROM kho WHERE id = '$_GET[id]'";
	...
	$errro = false;
	 if(isset($_POST['edit_warehouse'])){
		...
		if(empty($makho) || empty($tenkho) || empty($diachi) || empty($sodienthoai)){
			...
			$sql1 = "SELECT * FROM kho WHERE makho = '$makho'" ;
			$kq = $db->query($sql1);
			$rowkl = mysqli_num_rows($kq);
			if($maHH == $makho ){
				$sqlEHH = "UPDATE kho SET makho='$makho', tenkho='$tenkho', diachi='$diachi', sodienthoai='$sodienthoai', status='1' WHERE id = $id ";
				$db->update($sqlEHH);
				echo '<script>alert("Sửa thành công!"); window.location="../warehouse.php";</script>';
				}else if($rowkl > 0){
					$loii = "Mã kho hàng đã tồn tại";
				}else{
					$sqlEHH = "UPDATE kho SET makho='$makho', tenkho='$tenkho', diachi='$diachi', sodienthoai='$sodienthoai', status='1' WHERE id = $id ";
					$db->update($sqlEHH);
					echo '<script>alert("Sửa thành công!"); window.location="../warehouse.php";</script>';
						...
						<div class="form">
							...
							<div class="form__file">
								...
								<input type="hidden" id="id" name="txtId" placeholder="Mã Kho"
									...
									name="makho" placeholder="Mã Kho" value="<?php echo $maHH;?>">
							</div>
							<div class="form__file">
								<lable for="name">Tên kho <br>
								</lable>
								<input type="text" id="name" name="tenkho"
									placeholder="Tên Kho" value="<?php echo $tenHH;?>">
							</div>
							<div class="form__file">
