    ...
    if(isset($_POST['btn_kho'])){
		  ...
		  if(empty($makho) || empty($tenkho) || empty($diachi) || empty($sodienthoai)){
			...
			$rows         = mysqli_num_rows($result);
			  if($rows > 0){
				$loii = "Mã kho đã tồn tại!";
			  }else{
				$sqli = "INSERT INTO kho (makho, tenkho, diachi, sodienthoai, status) VALUES('$makho', '$tenkho', '$diachi', '$sodienthoai', '1')";
				$db->add($sqli);
				echo '<script>alert("Thêm mới kho hàng thành công!"); window.location="../warehouse.php";</script>';
			  }
		  }
    }
?>
					...
					<div class="form--add">
						<div class="form">
							...
							<div class="form__file">
								<lable for="mahang">Mã kho <br>
								</lable>
								<input type="text" id="mahang" name="makho"
									placeholder="Mã Kho" value="" required="required">
							</div>
							<div class="form__file">
								<lable for="name">Tên Kho <br>
								</lable>
								<input type="text" id="name" name="tenkho"
									placeholder="Tên Kho" value="" required="required">
							</div>
							<div class="form__file">
								<lable for="address">Địa chỉ <br>
								</lable>
								<input type="text" id="address" name="diachi"
									placeholder="Địa chỉ" value="" required>
							</div>
							<div class="form__file">
								<lable for="phone">Số điện thoại<br>
								</lable>
								<input type="number" id="phone" name="sodienthoai"
									placeholder="Số điện thoại" value="" required>
							</div>
							<div class="add-link">
								<button name="btn_kho" id="add">Thêm</button>
								<button id="cancel"><a style="color: black;" href="../warehouse.php">Thoát</a></button>
