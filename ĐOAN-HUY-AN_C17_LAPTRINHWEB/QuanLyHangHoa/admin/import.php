...
<?php
	...
	if(isset($_POST['timkiem'])){
		...
		if(empty($tuNgay) && empty($denNgay) && empty($tkcheck)){
			...
			$sqlIP = "SELECT * FROM import WHERE `dateimport` >= '$tuNgay' and `dateimport` <= '$denNgay'";
			$dataListIP = $db->getData($sqlIP);
		}else if(empty($tuNgay) && empty($denNgay)){
			$sqlIP = "SELECT * FROM import WHERE status = $tkcheck"; 
			$dataListIP = $db->getData($sqlIP);
...
<title>Quản Lý Nhập Hàng</title>
<!--Style CSS-->
	...
	<div class="wrapper">
		...
		<div class="table__txt d-flex">
				<div class="table__txt--title">
					<h2>Danh sách hóa đơn nhập</h2>
				</div>
				<ul class="table__txt--link d-flex">
					<li class="table__txt--item"><a>Hiện có <?php echo $numIP;?> phiếu nhập hàng hóa</a></li>
				</ul>
			</div>
			<div class="table__top d-flex">
				...
				<div class="table__top--add1">
					<a class="btn btn-link1" href="./import/add-import.php"> Tạo mới<i
					...
					<tbody>
						...
						?>
							<tr>
								...
								<td><?php $date = date_create($row['dateimport']); echo date_format($date,'d-m-Y');?></td>
								<?php
									$idnl= $row['nguoilap'];
								...
								<?php } ?>
								<td class="edit"><a href="./import/edit-import.php?id=<?php echo $row['id'];?>">
										<button id="edit">Sửa</button>
								</a></td>
								<td class="delete"><a href="./import/view-import.php?id=<?php echo $row["id"];?>">
