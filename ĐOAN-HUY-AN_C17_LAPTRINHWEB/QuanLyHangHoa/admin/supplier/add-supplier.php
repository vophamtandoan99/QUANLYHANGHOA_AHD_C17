...
<?php
    ...
    if(isset($_POST['btn_kho'])){
		  ...
		  if(empty($makho) || empty($tenkho) || empty($diachi) || empty($sodienthoai)){
			...
			$sql          = "select * from nhacungcap where maNCC = '$makho' ";
			$result       = $db->query($sql);
			$rows         = mysqli_num_rows($result);
			  if($rows > 0){
				$loii = "Mã nhà cung cấp đã tồn tại!";
			  }else{
				$sqli = "INSERT INTO nhacungcap (maNCC, tenNCC, diachi, sdt, status) VALUES('$makho', '$tenkho', '$diachi', '$sodienthoai', '1')";
				$db->add($sqli);
				echo '<script>alert("Thêm mới nhà cung cấp thành công!"); window.location="../supplier.php";</script>';
			  }
		  }
    }
?>
						...
						<div class="form">
							...
							<div class="form__file">
								...
								<input type="text" id="mahang" name="makho"
									placeholder="Mã Nhà Cung Cấp" value="" required="required">
							</div>
							<div class="form__file">
								<lable for="name">Tên nhà cung cấp <br>
								</lable>
								<input type="text" id="name" name="tenkho"
									placeholder="Tên Nhà Cung Cấp" value="" required="required">
							</div>
							<div class="form__file">
