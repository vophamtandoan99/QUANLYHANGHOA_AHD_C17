...
<?php 
	...
	$sql 		= "SELECT * FROM export WHERE id = '$_GET[id]'";   
	$dateimport = $db->getData($sql);

	$tenhang 	= $dateimport[0]["tenhangid"];
	$tenkho 	= $dateimport[0]["tenkhoid"];
	$tendaily 	= $dateimport[0]["dailyid"];
	$tenncc 	= $dateimport[0]["nccid"];
	$soluong 	= $dateimport[0]["soluong"];
	$gianhap 	= $dateimport[0]["giaxuat"];
	$tonggia 	= $dateimport[0]["tonggia"];
	$nguoilap 	= $dateimport[0]["ngxuat"];
	$trangthai 	= $dateimport[0]["status"];
	$dateimport = $dateimport[0]["ngayxuat"];
?>
<?php
	$sqlop = "SELECT us.fullname FROM user AS us INNER JOIN export ex ON us.id = ex.ngxuat WHERE ex.id = '$_GET[id]'";
	$dlkq = $db->query($sqlop);
	...
	<div class="wrapper">
		...
		<div class="table">
			...
			<div class="table__top ">
				<h2 style="text-align: center;">Thông tin chi tiết phiếu xuất</h2>
					...
					<div class="col-10 col-lg-7" style="margin: 0 auto;">
						...
						<div class="item-table" >
							<label for="" class="item__table" style="margin-right: 2rem;text-align: right;">Tên
								...
								<div class="input-items">
							<input type="text" value="<?php 
									$sqliDL = "SELECT * FROM daily WHERE id = $tendaily";
						...
						<div class="item-table" >
							<label for="" class="item__table" style="margin-right: 2rem;">Giá xuất
								:</label>
								<div class="input-items">
						...
						<div class="item-table" >
							<label for="" class="item__table" style="margin-right: 2rem;">Người xuất
								:</label>
								<div class="input-items">
		...
		<div style="margin: 20px; position:absolute;right: 0; bottom: 10px;" class="table--result" >
									<button id="back" class="btn btn-link3 result_link"
										style="width: 13rem;"><a href="../export.php">Trở lại</a></button>
								</div>
			</div>
		</div>
